...
import { AlertTriangle, Save, ArrowLeft, Loader2, Cpu, Hammer } from "lucide-react";
...
export default function DefectsPage() {
  ...
  const handleSaveDefect = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.machine_id || !formData.molde_id || !formData.quantity) {
    ...
    <div className="max-w-xl mx-auto space-y-8 p-4">
      ...
      <form onSubmit={handleSaveDefect} className="bg-white p-8 rounded-[2.5rem] border-2 border-gray-50 shadow-sm space-y-6">
        <div className="space-y-2">
          <label className="text-xs font-black text-gray-400 uppercase ml-2 flex items-center gap-2"><Cpu size={14}/> Máquina</label>
          <select 
            required
        ...
        <div className="space-y-2">
          <label className="text-xs font-black text-gray-400 uppercase ml-2 flex items-center gap-2"><Hammer size={14}/> Molde (Peça)</label>
          <select 
            required
        ...
        <div className="space-y-2">
          <label className="text-xs font-black text-gray-400 uppercase ml-2">Motivo / Observação</label>
          <textarea 
            ...
            placeholder="Ex: Peça manchada ou rebarba excessiva"
          />
        </div>
        ...
        >
          {isSubmitting ? <Loader2 className="animate-spin" /> : <><Save size={24} /> SALVAR DEFEITOS</>}
        </button>
      </form>
    </div>
  );
